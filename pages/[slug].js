...
export default function Post({ categories, post, preview }) {
  post = post[0];
                ...
                categories.map((category, key) => (
                  <div>
                    ...
                    {category.posts?.map((post, key) => (
                      <div
                        className={`text-sm md:text-base pt-2 cursor-pointer flex justify-between ${
                          currentPost == post.slug.current
                            ? "md:text-regi-red md:font-bold"
                            : "font-normal"
                        }`}
...
export async function getStaticProps({ params, preview = false }) {
  ...
  console.log(postQuery(params.slug));
  const post = await getClient(preview).fetch(postQuery(params.slug));
...
export async function getStaticPaths() {
  const slugs = await getClient(false).fetch(postSlugQuery);
  const paths = slugs.map((post) => ({ params: { slug: post.slug } }));
  return {
    paths: paths,
    fallback: false,
  };
